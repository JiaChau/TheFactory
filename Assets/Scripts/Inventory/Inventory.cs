...
public class Inventory : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // selectedSlot = new(null, 0, 0);
        //Call to refresh the inventory UI
        //OLD WAY WAS InventoryUI.Instance.RefreshInventoryUI();
    ...
    public bool AddItem(ItemData data, int amount = 1)
    {
        ...
        while (amount > 0 && slots.Count < maxSlots)
        {
            ...
            //slots.Add(new InventorySlot(data, toAdd, slots.Count));
            //// Subtract what we just added
            //amount -= toAdd;
            //// Flag as added
            //added = true;
    ...
    public void AddItemToHotBar(InventorySlot slot, int index)
    {
        hotBarSlots.RemoveAt(index);
        hotBarSlots.Insert(index, slot);
        //hotBarSlots[index].itemData = item;
        //hotBarSlots[index].amount = item.amount;
        HotBarUI.Instance.RefreshHotbarUI();
        // hotBarSlots[index].itemData.icon = item.icon;
    ...
    {
        hotBarSlots.RemoveAt(index);
        hotBarSlots.Insert(index, new InventorySlot(null, 0, index));
        HotBarUI.Instance.RefreshHotbarUI();
    }
